...
#include "simpleGA/simpleGA.h"
#include "scip/scip.h"
#include "atfmGA/heur_atfmGA.h"
...
void ShuffleX(int one, int two, double pGeneShuffle, vector<genotype> &population, int &numF)
//****************************************************************************80
// 
//  Purpose:
//
...
//
{
	...
	vector<int> geneindexes = ShuffleIndx(numF);

	//number of genes to shuffle
	int nGeneShuffle = pGeneShuffle * numF;
...
void crossover(int &seed, vector<genotype> &population, int &numF, int &nSols, int gen)
//****************************************************************************80
// 
//  Purpose:
//
	...
	for (mem = 0; mem < POPSIZE; ++mem)
	{
		...
		if (x < Pxover)
		{
			...
			if (first % 2 == 0) 
			{
				...
				//ShuffleX(one, mem, randPshuffle, population, numF); //UNCOMMENT TO USE

				//one and mem are indexes of the parents who are going to mate 
				//Xover1point(one, mem, seed, population, numF); //UNCOMMENT TO USE

				Xover2point(one, mem, seed, population, numF);
			...
			else
			{
				...
				if (mem == POPSIZE - 1)
				{
					...
					//ShuffleX(one, last, randPshuffle, population, numF); //UNCOMMENT TO USE

					//Xover1point(one, last, seed, population, numF); //UNCOMMENT TO USE

					Xover2point(one, last, seed, population, numF);
...
void initializePopulation(vector<genotype> &population, int &numF)
//****************************************************************************80
// 
//  Purpose:
//
...
//
{
	for (auto& p : population) //(p is an individual)
	{
		...
		//each individual/chromossome has size equal to the number of flights
		p.gene.resize(numF);
...
//  Discussion:
//
//    Randomly allocates departure slots to flights
//
//  Modified:
//
//    15 October 2018
...
void Xover1point(int father, int mother, int &seed, vector<genotype> &population, int &numF)
//****************************************************************************80
// 
//  Purpose:
//
...
//			  population		 genotype/GA space,
//			  numF				 number of feasible solutions.
//
{
	int i;
	int point;
	...
	//
	point = i4_uniform_ab(0, numF - 1, seed);
	//
...
void Xover2point(int father, int mother, int &seed, vector<genotype> &population, int &numF)
//****************************************************************************80
// 
//  Purpose:
//
...
//
{
	...
	//
	point1 = i4_uniform_ab(0, numF - 1, seed);
	point2 = i4_uniform_ab(0, numF - 1, seed);
